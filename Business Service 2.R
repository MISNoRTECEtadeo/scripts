...
setwd("S:/Esteban/NoRTEC Performance/2018-2019 Q4")
...
Data_bus <- read_xlsx("BusinessServicesPY1819_72919.xlsx", sheet = "ALL")
...
Count_NoRTEC_Distinct_Businesses <- Data_bus %>%
  group_by(COMPANY) %>%
  summarise(count=n_distinct(COMPANY,SERVICEOFFICE))
...
Second <- c("", "July 2018 through June 2019","","","","","","","","","")
...
Fifth <- c("TotalEmployers/Business Serrvices", NoRTEC_Uniq_Businesses, NoRTEC_All_Business_Services, 
...
write.xlsx(FINAL_Business_Services,file =  "Business_Services_18-19 (7-29-19).xlsx",colNames = TRUE)
